...
from igcaller import __version__
from igcaller.functions import (
    addMapQualAndScore,
    addPositionsAndOccurrences,
    assignPositionsToJandV,
    checkHomologyAndFunctionality,
    classSwitchAnalysis,
    collectBarcodesForCSR,
    collectBarcodesForEvents,
    convertSamToAnnotatedTable,
    findCombinationsJandV,
    findJandVgenes,
    getDsequence,
    getGeneralInfo,
    getIgTranslocations,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='IgCaller',
        description=f'IgCaller {__version__} (https://github.com/ferrannadeu/IgCaller)',
    )
    ...
    parser.add_argument('--singleCell',
                        ...
                        action="store_true",
                        default=False,
                        help="Enable single-cell mode: extract CB barcodes from reads and output per-event barcode tables")
    ...
    for GENE in ["IGH", "IGK", "IGL", "CSR"]:
        ...
        store, read_barcodes = convertSamToAnnotatedTable(miniSamT, chromGene, GENE)
        ...
        if GENE != "CSR":
            ...
            # Single-cell barcode output:
            if singleCell:
                barcode_rows = collectBarcodesForEvents(JV_list, information, read_barcodes, GENE)
                bcOut = open(miniSamT.replace("_miniSam.sam", "_output_"+GENE+"_barcodes.tsv"), "w")
                bcOut.write("Event\tMechanism\tRead_name\tCell_barcode\n")
                for row in barcode_rows:
                    bcOut.write("\t".join(row)+"\n")
                bcOut.close()
            ...
            # Single-cell barcode output for CSR:
            if singleCell:
                barcode_rows = collectBarcodesForCSR(JV_list, data, read_barcodes, bedFile, GENE)
                bcOut = open(miniSamT.replace("_miniSam.sam", "_output_"+GENE+"_barcodes.tsv"), "w")
                bcOut.write("Event\tMechanism\tRead_name\tCell_barcode\n")
                for row in barcode_rows:
                    bcOut.write("\t".join(row)+"\n")
    ...
    translocationsALL, translocationsPASS, onco_barcode_rows = getIgTranslocations(genomeVersion, inputsFolder, pathToSamtools, threadsForSamtools, bamT, bamN, chrom, coordsToSubset, tumorPurity, mntonco, mntoncoPass, mnnonco, mapqOnco, mncPoN, singleCell)

    Vseq = open(miniSamT.replace("_miniSam.sam", "_output_oncogenic_IG_rearrangements.tsv"), "w")
    Vseq.write("\n".join(translocationsALL))
    Vseq.close()

    # Single-cell barcode output for oncogenic IG rearrangements:
    if singleCell:
        bcOut = open(miniSamT.replace("_miniSam.sam", "_output_oncogenic_IG_rearrangements_barcodes.tsv"), "w")
        bcOut.write("Event\tRead_name\tCell_barcode\n")
        for row in onco_barcode_rows:
            bcOut.write("\t".join(row)+"\n")
        bcOut.close()
    ...
    comms = "rm "+wkDir+"/*.sam "+wkDir+"/*miniBam.bam "+wkDir+"/*miniBam.bam.bai "+wkDir+"/*miniBam_readsSpanningJV.bam "+wkDir+"/*miniBam_readsSpanningJV.bam.bai "+wkDir+"/*miniBam_readsSpanningBreak.bam "+wkDir+"/*miniBam_readsSpanningBreak.bam.bai "+wkDir+"/*_output_mpileup.tsv "+wkDir+"/*_readsSpanningJV.tsv "+wkDir+"/*_readsSpanningBreak.tsv "+wkDir+"/*_miniSam_splitinsert.tsv "+wkDir+"/*_miniSam_splitinsert_VJ.tsv"
    if len(glob.glob(wkDir+"/*MutPhased.bam")) == 1: comms = comms+" "+wkDir+"/*MutPhased.bam "+wkDir+"/*MutPhased.bam.bai "+wkDir+"/*MutPhased.tsv"
    subprocess.call(comms, shell=True)
